...
// Suggested uses
const SUGGESTED_USES = ['hero', 'detail', 'comparison', 'installation', 'social_media', 'banner', 'thumbnail'];
...
function CategoryBadge({ category }: { category: string }) {
  ...
  const colors: Record<string, string> = {
    historical_ad:
      'bg-amber-500/20 dark:bg-amber-500/30 text-amber-900 dark:text-amber-400 border-amber-500/30 dark:border-amber-500/20',
    product_hero:
      'bg-blue-500/20 dark:bg-blue-500/30 text-blue-900 dark:text-blue-400 border-blue-500/30 dark:border-blue-500/20',
    installation:
      'bg-green-500/20 dark:bg-green-500/30 text-green-900 dark:text-green-400 border-green-500/30 dark:border-green-500/20',
    detail:
      'bg-purple-500/20 dark:bg-purple-500/30 text-purple-900 dark:text-purple-400 border-purple-500/30 dark:border-purple-500/20',
    lifestyle:
      'bg-pink-500/20 dark:bg-pink-500/30 text-pink-900 dark:text-pink-400 border-pink-500/30 dark:border-pink-500/20',
    comparison:
      'bg-cyan-500/20 dark:bg-cyan-500/30 text-cyan-900 dark:text-cyan-400 border-cyan-500/30 dark:border-cyan-500/20',
  };

  return <Badge className={cn('text-xs', colors[category] || 'bg-muted')}>{info?.label || category}</Badge>;
}
...
function BrandImageCard({ image, onView, onDelete }: { image: BrandImage; onView: () => void; onDelete: () => void }) {
...
function UploadModal({
  ...
  const toggleSuggestedUse = (use: string) => {
    setSuggestedUses((prev) => (prev.includes(use) ? prev.filter((u) => u !== use) : [...prev, use]));
  };
    ...
    >
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Add images to help AI understand your brand's visual style.</DialogDescription>
        </DialogHeader>
...
export default function BrandImageLibrary({ embedded = false, selectedId }: BrandImageLibraryProps) {
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage your brand's visual assets for AI-powered ad generation</p>
        </div>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {/* Content */}
      <main className="container max-w-6xl mx-auto px-6 pt-24 pb-20 relative z-10">{content}</main>
    </div>
  );
}
