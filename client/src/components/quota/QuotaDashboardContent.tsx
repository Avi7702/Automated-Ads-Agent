  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        {quotaStatus && (
          <Badge variant="outline" className={cn('px-3 py-1', statusConfig[quotaStatus.status].color)}>
      ...
      {quotaStatus?.status === 'rate_limited' && quotaStatus.retryAfter && (
        <RateLimitCountdown retryAfter={quotaStatus.retryAfter} onComplete={() => refetchStatus()} />
      )}
      ...
      {quotaStatus?.warnings && quotaStatus.warnings.length > 0 && (
        <Alert
          variant="default"
          className="bg-yellow-500/10 dark:bg-yellow-500/20 border-yellow-500/30 dark:border-yellow-500/20"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <UsageChart data={history?.history || []} windowType={windowType} onWindowChange={setWindowType} />
        {breakdown && <UsageBreakdown data={breakdown} period={period} onPeriodChange={setPeriod} />}
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-card/50 backdrop-blur rounded-lg border p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge
                variant="outline"
                className="ml-auto text-xs bg-green-500/10 dark:bg-green-500/20 text-green-600 dark:text-green-400"
              >
                Real-time
              </Badge>
            </div>
            <p className="text-sm text-muted-foreground">
              Tracks all API calls made through this application. Updates instantly. The quota cards above show data
              from local tracking.
            </p>
          </div>
