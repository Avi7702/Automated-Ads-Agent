  ...
  const handleCancel = useCallback(
    async (postId: string) => {
      ...
      try {
        ...
        toast.success('Post cancelled', { description: 'The scheduled post has been cancelled.' });
      } catch (err: any) {
        toast.error('Failed to cancel', { description: err.message || 'Something went wrong. Please try again.' });
      } finally {
  ...
  const handleRetry = useCallback(
    async (postId: string) => {
      ...
      try {
        ...
        toast.success('Post rescheduled for retry', { description: 'The post has been queued for retry.' });
      } catch (err: any) {
        toast.error('Failed to retry', { description: err.message || 'Something went wrong. Please try again.' });
      } finally {
        setRetryingId(null);
      }
    },
