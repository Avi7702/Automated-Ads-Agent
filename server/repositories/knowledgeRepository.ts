...
export async function getBrandImagesForProducts(productIds: string[], userId: string): Promise<BrandImage[]> {
  ...
  // Optimized to use PostgreSQL array overlap operator (&&) for database-side filtering.
  // This avoids fetching all user images and filtering in memory, which is O(N).
  // Combined with a GIN index on product_ids, this query is significantly more efficient.
  return await db
    .select()
    .from(brandImages)
    .where(
