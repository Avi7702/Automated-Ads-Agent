...
export async function getBrandImagesForProducts(productIds: string[], userId: string): Promise<BrandImage[]> {
  ...
  // Optimized: Use database-side array overlap filtering instead of in-memory filtering.
  // This reduces memory usage and network payload by filtering results at the source.
  return await db
    .select()
    .from(brandImages)
    .where(
