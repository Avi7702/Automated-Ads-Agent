...
export async function updateAdSceneTemplate(id: string, updates: Partial<InsertAdSceneTemplate>): Promise<AdSceneTemplate> {
  const [template] = await db
...
export async function getPerformingAdTemplatesByCategory(userId: string, category: string): Promise<PerformingAdTemplate[]> {
  return await db
    .select()
    .from(performingAdTemplates)
    .where(
...
export async function getPerformingAdTemplatesByPlatform(userId: string, platform: string): Promise<PerformingAdTemplate[]> {
  // Optimization: Move filtering to database using arrayContains
  return await db
    .select()
    .from(performingAdTemplates)
    .where(
...
export async function updatePerformingAdTemplate(id: string, updates: Partial<InsertPerformingAdTemplate>): Promise<PerformingAdTemplate> {
  const [template] = await db
...
export async function searchPerformingAdTemplates(
  ...
  // Optimization: Move all filters to database level to reduce memory and payload size
  const conditions = [
    eq(performingAdTemplates.userId, userId),
    eq(performingAdTemplates.isActive, true)
  ];
