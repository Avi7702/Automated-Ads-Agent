...
export async function upsertQuotaMetrics(metrics: InsertGeminiQuotaMetrics): Promise<GeminiQuotaMetrics> {
  // Use atomic PostgreSQL UPSERT to reduce database round-trips and ensure concurrency safety
  const [result] = await db
    ...
    .onConflictDoUpdate({
      ...
      set: {
        ...
        // Accumulate models using JSONB merge operator (captures unique models in the window)
        modelBreakdown: sql`COALESCE(${geminiQuotaMetrics.modelBreakdown}, '{}'::jsonb) || ${JSON.stringify(metrics.modelBreakdown)}::jsonb`,
      },
    })
    .returning();
