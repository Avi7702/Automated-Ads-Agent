...
export class DbStorage implements IStorage {
  ...
  async getBrandImagesForProducts(productIds: string[], userId: string): Promise<BrandImage[]> {
    ...
    // and belong to the specified user - optimized with DB-level array overlap
    return await db
      .select()
      .from(brandImages)
      .where(
  ...
  async getPerformingAdTemplatesByPlatform(userId: string, platform: string): Promise<PerformingAdTemplate[]> {
    // Get all templates for user and filter by target platform - optimized with DB-level array check
    return await db
      .select()
      .from(performingAdTemplates)
      .where(
  ...
  async searchPerformingAdTemplates(
    ...
    // Start with base query conditions - optimized by moving all filters to DB level
    const conditions = [
      eq(performingAdTemplates.userId, userId),
      eq(performingAdTemplates.isActive, true)
    ];

    // Apply filters to DB query instead of in-memory
    if (filters.category) {
