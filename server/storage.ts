...
export class DbStorage implements IStorage {
  ...
  async getBrandImagesForProducts(productIds: string[], userId: string): Promise<BrandImage[]> {
    ...
    // Optimization: Use PostgreSQL array overlap operator (&&) for database-side filtering
    // This avoids fetching all user images and filtering in memory
    return await db
      .select()
      .from(brandImages)
      .where(
  ...
  async getPerformingAdTemplatesByPlatform(userId: string, platform: string): Promise<PerformingAdTemplate[]> {
    // Optimization: Use arrayContains for database-side filtering
    return await db
      .select()
      .from(performingAdTemplates)
      .where(
  ...
  async searchPerformingAdTemplates(
    ...
    // Optimization: Move all filtering to the database side to reduce payload and memory usage
    const conditions = [
      eq(performingAdTemplates.userId, userId),
      eq(performingAdTemplates.isActive, true)
    ];
  ...
  async getWeeklyBalance(userId: string): Promise<{ category: string; count: number }[]> {
    ...
    // Optimization: Use database-side aggregation (GROUP BY) instead of in-memory processing
    // This reduces payload size and offloads counting to the database engine
    return await db
      .select({
