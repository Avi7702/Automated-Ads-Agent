...
export class DbStorage implements IStorage {
  ...
  async getBrandImagesForProducts(productIds: string[], userId: string): Promise<BrandImage[]> {
    ...
    // Optimized: Use PostgreSQL && (overlap) operator to filter at the database level.
    // This avoids fetching all user images into memory and filtering in JavaScript.
    return await db
      .select()
      .from(brandImages)
      .where(
  ...
  async getWeeklyBalance(userId: string): Promise<{ category: string; count: number }[]> {
    ...
    // Optimized: Use GROUP BY and COUNT in SQL to perform aggregation at database level.
    // This reduces data transfer and memory usage in the application.
    const results = await db
      .select({
