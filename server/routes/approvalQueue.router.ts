...
export const approvalQueueRouter: RouterFactory = (ctx: RouterContext): Router => {
  ...
  router.get('/', requireAuth, asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ module: 'ApprovalQueue', err: error }, 'Failed to fetch queue items');
      res.status(500).json({
        success: false,
   ...
   */
  router.post('/:id/revision', requireAuth, asyncHandler(async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { notes } = req.body;
   ...
   */
  router.post('/bulk-approve', requireAuth, asyncHandler(async (req: Request, res: Response) => {
    try {
      const { ids } = req.body;
   ...
   */
  router.get('/:id/audit', requireAuth, asyncHandler(async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
   ...
   */
  router.get('/settings', requireAuth, asyncHandler(async (req: Request, res: Response) => {
    try {
   ...
   */
  router.put('/settings', requireAuth, asyncHandler(async (req: Request, res: Response) => {
    try {
      const {
        autoApproveEnabled,
