...
/** Create FunctionTool instances for product operations */
export function createProductTools(storage: IStorage): FunctionTool[] {
  ...
  const selectProducts = new FunctionTool({
    ...
    parameters: z.object({
      productIds: z.array(z.string()).describe('Array of product IDs to select (1-6)'),
    }),
    execute: async (input) => {
      try {
        const products = [];
