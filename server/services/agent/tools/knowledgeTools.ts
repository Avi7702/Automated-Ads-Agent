...
export const declarations: FunctionDeclaration[] = [
  {
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: Type.STRING,
          description: 'Search query (e.g. "brand color palette", "product warranty terms")',
        },
      },
      required: ['query'],
    },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        category: {
          type: Type.STRING,
          description: 'Filter by category (e.g. "product_showcase", "lifestyle")',
        },
      },
    },
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        productId: { type: Type.STRING, description: 'The product ID to get enhanced knowledge for' },
      },
      required: ['productId'],
    },
  },
...
export function createExecutors(storage: IStorage): Map<string, ToolExecutor> {
  ...
  const executeVaultSearch: ToolExecutor = async (args, _userId) => {
    try {
  ...
  map.set('vault_search', executeVaultSearch);
  map.set('search_knowledge_base', executeVaultSearch);

  map.set('get_templates', async (args, _userId) => {
    try {
      ...
      return {
        ...
        message: `Found ${templates.length} template(s)${category ? ` in category "${category}"` : ''}.`,
      };
    } catch (err: unknown) {
  ...
  map.set('get_product_knowledge', async (args, userId) => {
    if (!userId) {
    ...
    try {
      ...
      const context = await productKnowledgeService.buildEnhancedContext(productId, userId);
