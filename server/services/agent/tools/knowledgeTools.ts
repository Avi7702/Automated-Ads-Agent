...
/** Create FunctionTool instances for knowledge base and template operations */
export function createKnowledgeTools(storage: IStorage): FunctionTool[] {
  const executeVaultSearch = async (input: { query: string }) => {
    try {
  ...
  const vaultSearchParams = z.object({
    query: z.string().describe('Search query (e.g. "brand color palette", "product warranty terms")'),
  });
