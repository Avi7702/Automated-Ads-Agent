...
// Mock storage with in-memory implementations
vi.mock('../storage', () => {
  const adCopies = new Map<string, Record<string, unknown>>();
  ...
  return {
    storage: {
      ...
      saveGeneration: vi.fn().mockImplementation((data: Record<string, unknown>) => {
        const id = nextId();
        const gen = { id, ...data, createdAt: new Date() };
        generations.set(id, gen);
        return Promise.resolve(gen);
      }),
      saveAdCopy: vi.fn().mockImplementation((data: Record<string, unknown>) => {
        const id = nextId();
        const copy = { id, ...data, createdAt: new Date() };
        adCopies.set(id, copy);
        return Promise.resolve(copy);
      }),
      getAdCopyByGenerationId: vi.fn().mockImplementation((generationId: string) => {
        const results: Record<string, unknown>[] = [];
        for (const copy of adCopies.values()) {
          if (copy['generationId'] === generationId) results.push(copy);
        }
        return Promise.resolve(results);
      }),
      getAdCopyById: vi.fn().mockImplementation((id: string) => {
        return Promise.resolve(adCopies.get(id));
      }),
      deleteAdCopy: vi.fn().mockImplementation((id: string) => {
        adCopies.delete(id);
        return Promise.resolve(undefined);
      }),
      updateUserBrandVoice: vi.fn().mockImplementation((userId: string, brandVoice: unknown) => {
        const user = users.get(userId);
        if (user) {
          user['brandVoice'] = brandVoice;
          users.set(userId, user);
        }
        return Promise.resolve(user ?? { id: userId, brandVoice });
      }),
    },
  };
});
