...
async function auditProducts() {
  ...
  for (const [category, prods] of byCategory) {
    ...
    for (const p of prods) {
      ...
      console.log(
        `${name} |  ${checks.desc}   |  ${checks.feat}   |  ${checks.bene}   |  ${checks.spec}   |  ${checks.tags}   |  ${checks.sku}   | ${enrichStatus} |  ${relCount}  |  ${scenCount}`,
      );
    }
  }
...
`);
  ...
  const needsWork = allProducts.filter(
    (p) => !p.description || !p.features || !p.benefits || !p.specifications || !p.tags || !p.sku,
  );
