...
async function createTables() {
  ...
  try {
    ...
    console.log('\nCreating indexes...');
    await db.execute(
      sql`CREATE INDEX IF NOT EXISTS "idx_uploads_status" ON "ad_analysis_uploads" USING btree ("status")`,
    );
    await db.execute(
      sql`CREATE INDEX IF NOT EXISTS "idx_uploads_expires" ON "ad_analysis_uploads" USING btree ("expires_at")`,
    );
    await db.execute(
      sql`CREATE INDEX IF NOT EXISTS "idx_uploads_user_id" ON "ad_analysis_uploads" USING btree ("user_id")`,
    );
    await db.execute(
      sql`CREATE INDEX IF NOT EXISTS "idx_learned_patterns_user_id" ON "learned_ad_patterns" USING btree ("user_id")`,
    );
    await db.execute(
      sql`CREATE INDEX IF NOT EXISTS "idx_learned_patterns_category" ON "learned_ad_patterns" USING btree ("category")`,
    );
    await db.execute(
      sql`CREATE INDEX IF NOT EXISTS "idx_learned_patterns_platform" ON "learned_ad_patterns" USING btree ("platform")`,
    );
    await db.execute(
      sql`CREATE INDEX IF NOT EXISTS "idx_history_pattern_id" ON "pattern_application_history" USING btree ("pattern_id")`,
    );
    await db.execute(
      sql`CREATE INDEX IF NOT EXISTS "idx_history_user_id" ON "pattern_application_history" USING btree ("user_id")`,
    );
