...
import {
  ...
  _runEnrichmentForPendingProducts,
...
async function main(): Promise<void> {
  ...
  // BATCH MODE
  // ============================================
  console.log(args.all ? '🔄 Re-enriching ALL products...\n' : '🔄 Enriching pending products...\n');
...
function printResult(result: EnrichmentOutput, verbose: boolean): void {
  ...
  if (verbose) {
    ...
    console.log(
      `    Claims Verified: ${g4.truthChecks.filter((t) => t.verdict === 'VERIFIED').length}/${g4.truthChecks.length}`,
    );
...
function printSummary(results: Map<string, EnrichmentOutput>, durationSeconds: number): void {
  console.log('\n' + '═'.repeat(50));
